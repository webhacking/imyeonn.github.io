...
사실 이 때의 경험이 IA와의 첫번째 만남이었고 시행착오의 시작이었다. 작년 이맘때 쯤 프로젝트 관리학 전공수업에서 IA라는 용어를 처음 접했다. 그 때 IA를 그려오라는 과제에서 C를 받았던 게 IA와의 악연의 계기였던 것 같다.
...
이제와서 되돌아보면, 과거에 제작된 웹들은 계층구조를 따르는 게 대부분이었지만 최근 웹이나 모바일앱들은 깔끔하고 보기좋게 정리되지 않았기 때문에 IA를 그리는 게 어려웠던 것 같다. 계층구조는 IA의 패턴 중 하나일 뿐인데, 또 IA도 서비스를 설계하기 위한 보조도구일 뿐인데, 방법론에만 너무 매달렸다.
...
주로 Hierarchy나 Hub-and-spoke의 하위 페이지로 사용되지만 사용자가 섹션 사이를 오갈 때 빙 돌아가야하기 때문에 콘텐츠를 탐험하는데 오히려 장애물이 될 수 있음을 유의해야 한다.
